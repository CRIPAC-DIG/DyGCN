...
# from model.dynGCN_2hop import DynGCN
...
def main(_):
    ...
    with graph.as_default():
        trainm = GCN(config, device, loader, "Train")
        testm = GCN(config, device, loader, "Valid")
    ...
    with tf.Session(graph=graph, config=session_config) as session:
        ...
        for epoch in range(config.epoch_num):
            ...
            INFO_LOG("Epoch %d  Train " % epoch + str(eavluation_result), epoch % 10 == 0)
            INFO_LOG("Epoch %d Train costs %.5f" %
            ...
            INFO_LOG("Epoch %d  Valid " % epoch + str(eavluation_result), epoch % 10 == 0)
            INFO_LOG("Epoch %d Valid cost %.5f" % (epoch, cost), epoch % 10 == 0)
            sum_time_consume += eavluation_result["time_consume"]
            INFO_LOG("Epoch %d &&&&time comsume %.5f" % (epoch, sum_time_consume), epoch % 1 == 0)
            # #
            ...
            if best_f1_score < f1_score:
                ...
                best_epoch = epoch
                # DynGCN_saver(session, config, best_f1_score, best_epoch, "ARTI")
            ...
            INFO_LOG("*** best f1_score now is %.3f in %d epoch" % (best_f1_score, best_epoch), epoch % 10 == 0)
            INFO_LOG("Epoch %d  Valid " % epoch + str(eavluation_result), epoch % 10 == 0)

            if epoch % 1 == 0 and epoch != 0:
                # change_2_next_graph_date(loader, trainm, testm, session)
                loader.change_2_next_graph_date()
                # load_last_time_embedding contains the dyn adj
                # trainm.load_last_time_embedding(loader.present_graph, session)
