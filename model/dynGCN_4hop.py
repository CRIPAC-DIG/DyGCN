...
class DynGCN(object):
    def __init__(self, config, device, loader, mode):
        ...
        with tf.device(device), tf.name_scope(mode), tf.variable_scope("DynGCN", reuse=reuse):
            ...
            four_order_hash = tf.cast(tf.cast(tf.matmul(adj_now, tf.reshape(three_order_hash, (node_num,1))), dtype=tf.bool), dtype=tf.float32)
            four_order_hash = tf.reshape(four_order_hash, (node_num,))
            four_order_hash = tf.cast(tf.cast(four_order_hash - one_order_hash > 0, dtype=tf.bool), dtype=tf.float32) 
            four_order_hash = tf.cast(tf.cast(four_order_hash - two_order_hash > 0, dtype=tf.bool), dtype=tf.float32) 
            four_order_hash = tf.cast(tf.cast(four_order_hash - three_order_hash > 0, dtype=tf.bool), dtype=tf.float32) 
            ...
            self.embedding4order = self.gcn_4hop(adj_now, self.embedding3order, self.embedding2order, four_order_hash, n_layer, resue_id=0)

            self.final_embedding = self.embedding4order 
    ...
    def gcn_4hop(self, adj, then_embedding_x, original_embedding_x, four_order_hash, n_layer=1, resue_id=0):
        delta_H = then_embedding_x - original_embedding_x

        four_order_hash_embedd = tf.reshape(tf.stack([four_order_hash] * self.hidden_size, axis=1), (self.node_num, self.hidden_size))

        H0 = delta_H

        for idx in range(n_layer):

            gcn_w_4hop_idx = self.weights("gcn_w_4hop", self.hidden_size, 0, resue_id)
            gcn_w_self_4hop_idx = self.weights("gcn_self_w_4hop", self.hidden_size, 0, resue_id)

            H1 = tf.nn.relu(tf.matmul(tf.matmul(adj, H0), gcn_w_4hop_idx) + tf.matmul(then_embedding_x, gcn_w_self_4hop_idx))

            H0 = H1

        final_4order_embedding =  four_order_hash_embedd * H0 + (1 - four_order_hash_embedd) * then_embedding_x

        return final_4order_embedding
