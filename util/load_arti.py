...
class Loader(object):
    def __init__(self, flag):
        if platform.system() == 'Linux':
            self.path_init_file = path_init_file = '/home/cuizeyu/pythonfile/dynGCN/data/data_arti_new/'
            self.path_file = path_file = './data/data_arti_new/'
            # path_init_file = '/home/hufenyu/Documents/pythonfile/dataset/'
            # path_file = '/home/hufenyu/Documents/pythonfile/lightrnn_czy/data/'

        else:
            self.path_init_file = path_init_file = '/Users/czy_yente/PycharmProjects/dynGCN/data/data_arti_new/'
            self.path_file = path_file = './data/data_arti_new/'

        with open(path_init_file + "node2id.json", 'r') as f:
            self.node2id = node2id = json.load(f)

        self.embedding_path = 'arti_new_embedding/'
        self.present_graph = 2
        # print "list_gpickle", os.listdir(path_init_file)
        all_file = filter(lambda x: x[-8:] == '.gpickle' and x[0] != '.', os.listdir(path_init_file))
        # print all_file
